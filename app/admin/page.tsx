...
// Tipe data untuk satu properti (sesuaikan dengan struktur tabel properties)
type Property = {
  id: string;
  name: string;
  company_name?: string;
...
export default function AdminControlTower() {
  ...
  const [currentSavedLogo, setCurrentSavedLogo] = useState('');
...
if (logoFile) {
  ...
  if (uploadedUrl) {
    finalLogoUrl = uploadedUrl;
      ...
      if (editingPropId) {
        ...
        setProperties((prev) =>
          prev.map((p) => (p.id === editingPropId ? updated : p))
        );
        setStatus({
          type: 'success',
          msg: `Data Properti "${updated.name}" Berhasil Diperbarui!`,
        });
      } else {
        ...
        setStatus({
          type: 'success',
          msg: `Properti "${newProp.name}" Berhasil Terdaftar!`,
        });
      }
      resetPropertyForm();
    } catch (err: any) {
  ...
  const handleCreateUser = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setStatus({
        type: 'success',
        msg: `User "${username}" Berhasil Dibuat!`,
      });
    } catch (err: any) {
      setStatus({ type: 'error', msg: err.message });
    }
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col justify-center items-center text-white font-mono p-6">
        ...
        <p className="text-[10px] text-slate-400 mt-2 uppercase">
          Halaman ini hanya untuk otoritas tertinggi (Super Admin).
        </p>
      </div>
    );
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="border-b border-slate-200 pb-4 flex justify-between items-end">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-slate-900 text-white font-bold uppercase rounded hover:bg-sky-700 transition-colors"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white border border-slate-200 p-6 rounded-xl shadow-sm space-y-6">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              ...
              {editingPropId && (
                <button
                  ...
                  className="flex items-center gap-1 text-[10px] font-bold uppercase text-red-600 bg-red-50 px-2 py-1 rounded border border-red-100"
                >
            ...
            <form onSubmit={handlePropertySubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="font-bold text-slate-500 uppercase">
                  Nama Properti (Sistem) <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  value={propName}
                  onChange={(e) => setPropName(e.target.value)}
                  className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded font-bold"
                  placeholder="CONTOH: Grand Santhi Hotel"
                  required
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded font-bold"
                  placeholder="CONTOH: PT. Santhi Bali Property"
                  disabled={isUploading}
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded font-bold resize-none h-16"
                  placeholder="Jalan Patih Jelantik No. 1..."
                  disabled={isUploading}
                />
              </div>
                ...
                <div className="flex items-center gap-4 p-4 bg-slate-50 border border-dashed border-slate-300 rounded-lg relative hover:bg-slate-100/70 transition-all">
                  ...
                  <div className="space-y-0.5 flex-grow">
                    ...
                    <p className="text-[9px] text-slate-400 uppercase font-bold">
                      Format PNG/JPG ukuran direkomendasikan kotak
                    </p>
                  </div>
                  <input
                    type="file"
                ...
                {(logoPreviewUrl || currentSavedLogo) && (
                  <div className="p-3 bg-slate-900 rounded-lg flex items-center gap-4 border border-slate-800">
                    <div className="w-12 h-12 bg-white rounded border border-slate-700 p-1 flex items-center justify-center overflow-hidden shrink-0">
                      <img
                        src={logoPreviewUrl || currentSavedLogo}
                        alt="Preview Logo"
                    ...
                    <div>
                      <p className="text-[10px] font-black text-amber-400 uppercase tracking-wider">
                        {logoPreviewUrl ? 'Berkas Baru Siap Diupload' : 'Berkas Terarsip di Server'}
                      </p>
                      <p className="text-[9px] text-slate-400 font-bold mt-0.5 uppercase truncate max-w-xs">
                        {logoFile ? logoFile.name : 'Using Active Logo'}
                      </p>
                    </div>
                  </div>
                )}
              ...
              <button
                type="submit"
                disabled={isUploading}
                className={`w-full text-white py-3 rounded font-black uppercase tracking-wider transition-colors ${
                  editingPropId ? 'bg-amber-600 hover:bg-amber-700' : 'bg-slate-950 hover:bg-sky-700'
                } disabled:opacity-50`}
              >
            ...
            <div className="pt-4 border-t border-slate-100">
              <p className="font-bold text-slate-400 uppercase border-b pb-1 mb-2">
                Daftar Klien Terkoneksi ({properties.length})
              </p>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {p.address && (
                          <p className="text-[9px] text-slate-500 font-bold mt-0.5">{p.address}</p>
                        )}
                      </div>
                    </div>
                    <button
                      onClick={() => startEditProperty(p)}
                      className="flex items-center gap-1 text-[10px] bg-white border border-slate-300 font-black text-slate-600 px-2 py-1 rounded hover:bg-amber-50 hover:text-amber-700 hover:border-amber-300 transition-all uppercase shadow-sm"
                    >
                      <Edit2 size={10} /> Edit
                    </button>
                  </div>
            ...
            <form onSubmit={handleCreateUser} className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded font-bold"
                  placeholder="CONTOH: admin@grandsanthi"
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    className="w-full p-2.5 bg-white border border-slate-200 font-bold rounded cursor-pointer"
                  >
                ...
                <div className="space-y-1">
                  <label className="font-bold text-slate-500 uppercase">
                    Plotting Hotel <span className="text-red-500">*</span>
                  </label>
                  <select
                    value={selectedProp}
                    onChange={(e) => setSelectedProp(e.target.value)}
                    className="w-full p-2.5 bg-white border border-slate-200 font-bold rounded cursor-pointer uppercase"
                  >
              ...
              <button
                type="submit"
                className="w-full bg-slate-950 text-white py-3 rounded font-black uppercase tracking-wider hover:bg-emerald-700 transition-colors"
              >
                Generate Akun Klien
              </button>
            </form>
