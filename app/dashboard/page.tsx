...
import { Activity, Users, Briefcase, Percent, FileText, Database, Building2, Lock, LayoutGrid, Edit3, Settings, LogOut } from 'lucide-react';
...
function getAggregatedData(year, monthStr, isYTD, dbData, activePropId) {
  const targetMonth = parseInt(monthStr);
  const targetYear = parseInt(year);
  
  const rows = dbData.filter(d => 
  ...
  let sum = {
    avail_act: 0, avail_bud: 0, sold_act: 0, sold_bud: 0, pax_act: 0, pax_bud: 0, paid_act: 0,
    rev_room_act: 0, rev_room_bud: 0, rev_fb_act: 0, rev_fb_bud: 0, rev_meet_act: 0, rev_meet_bud: 0, rev_oth_act: 0, rev_oth_bud: 0,
    cost_fb_act: 0, cost_fb_bud: 0, payroll_act: 0, payroll_bud: 0, other_exp_act: 0, other_exp_bud: 0, energy_act: 0, energy_bud: 0,
    non_op_act: 0, non_op_bud: 0, gop_act: 0, gop_bud: 0
  };
  ...
  const totalRevAct = sum.rev_room_act + sum.rev_fb_act + sum.rev_meet_act + sum.rev_oth_act;
  const totalRevBud = sum.rev_room_bud + sum.rev_fb_bud + sum.rev_meet_bud + sum.rev_oth_bud;
  const totalExpAct = sum.cost_fb_act + sum.payroll_act + sum.other_exp_act + sum.energy_act;
  
  return {
    ...sum, totalRevAct, totalRevBud, totalExpAct,
...
function extractVal(data, dept, isBudget) {
  ...
  switch(dept) {
    ...
    case "STAT_OCC": return isBudget ? data.avail_bud : data.sold_act; 
...
const formatRp = (num) => new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(Math.round(num || 0));
const formatNum = (num) => new Intl.NumberFormat('id-ID', { minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(Math.round(num || 0));
...
export default function Dashboard() {
  ...
  const [activePropLogo, setActivePropLogo] = useState(''); 
  const [userRole, setUserRole] = useState('');
  const [propList, setPropList] = useState([]);
  const [isAuthChecking, setIsAuthChecking] = useState(true);

  const [compareMode, setCompareMode] = useState("BUDGET"); 
  const [compareYear, setCompareYear] = useState("2024");
  const [activeDept, setActiveDept] = useState("TOTAL_REVENUE"); 
  ...
  useEffect(() => {
    ...
    async function fetchLedgerData() {
      setIsLoadingDB(true);
      const yearsToFetch = compareMode === "YOY" ? [parseInt(selectedYear), parseInt(compareYear)] : [parseInt(selectedYear)];
  ...
  const indexMenuRev = [
    { id: "TOTAL_REVENUE", label: "TOTAL REVENUE" }, { id: "ROOM", label: "ROOM REVENUE" }, { id: "FB", label: "F&B REVENUE" },
    { id: "MEETING", label: "MEETING REVENUE" }, { id: "OTHERS", label: "OTHER REVENUE" }
  ];
  const indexMenuCost = [
    { id: "COST_FB", label: "COST OF F&B" }, { id: "PAYROLL", label: "PAYROLL" }, { id: "ENERGY", label: "ENERGY COST" },
    { id: "OTHER_EXP", label: "OTHER EXPENSES" }, { id: "NON_OP", label: "NON OPERATING" }
  ];
  const indexMenuGop = [ { id: "GOP", label: "GROSS OPERATING PROFIT" } ];
  const indexMenuStat = [
    { id: "STAT_OCC", label: "OCCUPANCY & ROOM SOLD" }, { id: "STAT_ADR", label: "ADR (AVG DAILY RATE)" },
    { id: "STAT_REVPAR", label: "REVPAR" }, { id: "STAT_PAX", label: "TOTAL GUEST (PAX)" }
  ];

  const lblDept = {
    "TOTAL_REVENUE": "Total Revenue", "ROOM": "Room Revenue", "FB": "F&B Revenue", "MEETING": "Meeting Revenue", "OTHERS": "Other Revenue",
    "COST_FB": "Cost of F&B", "PAYROLL": "Payroll", "OTHER_EXP": "Other Expenses", "ENERGY": "Energy Cost", "NON_OP": "Non Operating", "GOP": "Gross Operating Profit",
    "STAT_OCC": "Occupancy & Room Sold", "STAT_ADR": "ADR (Avg Daily Rate)", "STAT_REVPAR": "RevPAR", "STAT_PAX": "Total Guest (Pax)"
  };

  const isRpFormat = !["STAT_OCC", "STAT_PAX"].includes(activeDept);
  const isCostOrGop = ["COST_FB", "PAYROLL", "OTHER_EXP", "ENERGY", "NON_OP", "GOP"].includes(activeDept);
  const isStat = activeDept.startsWith("STAT_");
  const PIE_COLORS = ['#ffc107', '#10b981']; 

  const renderDashboardPanel = (yearStr, isCompact) => {
    ...
    // ---- PERUBAHAN DI SINI: LOGIKA barChartData UNTUK TOTAL_REVENUE ----
    const barChartData = (() => {
  ...
  if (isYTD) {
    return monthsList.map(m => {
      ...
      return {
          ...
          // Bulanan + TOTAL REVENUE: tetap 4 batang agregat
          return [
      ...
      // Untuk departemen lain, tetap seperti sebelumnya
      if (!isYTD) {
      ...
      <div className={`flex ${isCompact ? 'flex-col' : 'flex-col md:flex-row'} gap-4 2xl:gap-6`}>
        ...
        <div className={`w-full ${isCompact ? '' : 'md:w-1/3'} flex flex-col space-y-3 bg-white p-3 2xl:p-5 border border-slate-200 rounded-lg shadow-sm overflow-y-auto`}>
          <div className="border-b border-slate-200 pb-1.5 mb-1">
            <p className="text-[10px] 2xl:text-xs font-black text-sky-800 uppercase">PERFORMA {yearStr}</p>
          </div>
          
          <div className="grid grid-cols-2 gap-2 2xl:gap-4">
            <div>
              <p className="text-[8px] 2xl:text-[9px] text-slate-500 font-bold uppercase mb-0.5">Actual Result</p>
              <p className={`${isCompact ? 'text-sm' : 'text-base 2xl:text-lg'} font-bold text-slate-950 truncate`}>{isRpFormat ? formatRp(actualValue) : formatNum(actualValue)}</p>
            </div>
            <div>
              <p className="text-[8px] 2xl:text-[9px] text-slate-500 font-bold uppercase mb-0.5">{activeDept === "STAT_OCC" ? "Target (Avail)" : "Budget Target"}</p>
              <p className={`${isCompact ? 'text-xs' : 'text-sm 2xl:text-md'} font-bold text-slate-600 truncate`}>{isRpFormat ? formatRp(budgetValue) : formatNum(budgetValue)}</p>
            </div>
          </div>
          ...
          <div className="bg-slate-100 border border-slate-200 p-2 2xl:p-3 rounded-lg shadow-inner">
            ...
            <p className={`text-sm 2xl:text-lg font-bold truncate ${variance >= 0 ? (isCostOrGop && activeDept !== "GOP" ? 'text-red-600' : 'text-emerald-600') : (isCostOrGop && activeDept !== "GOP" ? 'text-emerald-600' : 'text-red-600')}`}>
              {variance >= 0 ? '+' : ''}{isRpFormat ? formatRp(variance) : formatNum(variance)}
            </p>
          </div>

          {isCostOrGop && (
            <div className="pt-1.5 2xl:pt-2 border-t border-slate-200 mt-1">
              <p className="text-[8px] 2xl:text-[9px] text-slate-500 font-bold uppercase mb-1">Ratio to Total Revenue</p>
                  ...
                  <table className="w-full text-[9px] 2xl:text-[10px] font-bold">
                    <tbody>
                        <tr className="border-b border-slate-200">
                          <td className="py-1 2xl:py-1.5 px-2 text-slate-500 bg-slate-200/60">Total Revenue</td>
                          <td className="py-1 2xl:py-1.5 px-2 text-right text-slate-800 bg-slate-200/60">{formatRp(data.totalRevAct)}</td>
                        </tr>
                        <tr>
                          <td className="py-1 2xl:py-1.5 px-2 text-slate-500">{activeDept === "GOP" ? "GOP" : "Cost"}</td>
                          <td className="py-1 2xl:py-1.5 px-2 text-right text-slate-900">{formatRp(actualValue)}</td>
                        </tr>
                    </tbody>
                  </table>
              </div>
          ...
          {!isStat && !isCostOrGop && activeDept === "TOTAL_REVENUE" && (
            <>
              <div className="pt-2 border-t border-slate-200 space-y-1 mt-1">
                <p className="text-[8px] 2xl:text-[9px] text-slate-500 font-bold uppercase mb-1">Revenue Breakdown</p>
                {[
                  { lbl: "Room", act: data.rev_room_act }, { lbl: "F&B", act: data.rev_fb_act },
                  { lbl: "Meeting", act: data.rev_meet_act }, { lbl: "Others", act: data.rev_oth_act }
                ].map((item, idx) => {
                  const pct = data.totalRevAct > 0 ? (item.act / data.totalRevAct) * 100 : 0;
                  return (
                    <div key={idx} className="flex justify-between items-center border-b border-slate-100 pb-1">
                      <span className="text-slate-600 text-[9px] 2xl:text-[10px] truncate max-w-[60px] 2xl:max-w-none">{item.lbl}</span>
                      <div className="text-right flex items-center shrink-0">
                        <span className="text-slate-800 text-[9px] 2xl:text-[10px] font-bold">{formatRp(item.act)}</span>
                        <span className="text-sky-700 text-[9px] 2xl:text-[10px] ml-2 font-black w-8 text-right">{pct.toFixed(1)}%</span>
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="pt-2 border-t border-slate-200 space-y-1 mt-2">
                <p className="text-[8px] 2xl:text-[9px] text-slate-500 font-bold uppercase mb-1">Efficiency Ratio (Rev vs Cost)</p>
                <div className="bg-slate-50 rounded-md border border-slate-200 overflow-hidden">
                  <table className="w-full text-[9px] 2xl:text-[10px] font-bold">
                    <tbody>
                        <tr className="border-b border-slate-200">
                          <td className="py-1 2xl:py-1.5 px-2 text-slate-500 bg-emerald-50">Total Revenue</td>
                          <td className="py-1 2xl:py-1.5 px-2 text-right text-emerald-800 bg-emerald-50">{formatRp(data.totalRevAct)}</td>
                        </tr>
                        <tr className="border-b border-slate-200">
                          <td className="py-1 2xl:py-1.5 px-2 text-slate-500 bg-red-50">Total Expense</td>
                          <td className="py-1 2xl:py-1.5 px-2 text-right text-red-800 bg-red-50">{formatRp(data.totalExpAct)}</td>
                        </tr>
                        <tr>
                          <td className="py-1 2xl:py-1.5 px-2 text-slate-800 font-black">GOP Margin</td>
                          <td className="py-1 2xl:py-1.5 px-2 text-right text-slate-900 flex justify-end items-center gap-2">
                             ...
                             <span className="text-sky-700 font-black w-8 text-right">
                                {data.totalRevAct > 0 ? ((data.gop_act / data.totalRevAct) * 100).toFixed(1) : 0}%
                             </span>
                          </td>
                        </tr>
                    </tbody>
          ...
          {isStat && (
            <div className="pt-2 border-t border-slate-200 mt-1 space-y-1.5">
              <p className="text-[8px] 2xl:text-[9px] text-slate-500 font-bold uppercase mb-1">Statistic Summary</p>
              {[
                ...
                { lbl: "Total Guest", val: formatNum(data.pax_act) }
                  ...
                  <div key={i} className="flex justify-between border-b border-slate-100 pb-1">
                    <span className="text-slate-600 text-[9px] 2xl:text-[10px]">{s.lbl}</span>
                    <span className="text-slate-800 font-bold text-[9px] 2xl:text-[10px]">{s.val}</span>
                  </div>
              ))}
            </div>
          )}
        ...
        {/* PANEL GRAFIK - PERUBAHAN DI BAGIAN RENDER BAR */}
        <div className={`w-full ${isCompact ? 'h-[200px] lg:h-[220px] 2xl:h-[300px]' : 'md:w-2/3 border-l border-slate-200 pl-4 2xl:pl-6 h-[220px] lg:h-[250px] 2xl:h-[360px]'}`}>
          <ResponsiveContainer width="100%" height="100%">
            {isCostOrGop ? (
              <PieChart>
                ...
                <Tooltip formatter={(value) => formatRp(value)} contentStyle={{ backgroundColor: 'white', border: '1px solid #e2e8f0', borderRadius: '6px' }} />
                <Legend verticalAlign="bottom" height={24} iconType="circle" wrapperStyle={{ fontSize: '9px', fontWeight: 'bold', color: '#1e293b' }} />
              </PieChart>
            ) : (
              <BarChart data={barChartData} margin={{ top: 0, right: 0, left: -15, bottom: 0 }} barGap={4}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                <XAxis dataKey="name" tick={{ fill: '#64748b', fontSize: 9 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: '#64748b', fontSize: 9 }} axisLine={false} tickLine={false} tickFormatter={(v) => !isRpFormat ? formatNum(v) : `Rp${(v/1000000).toFixed(0)}M`} />
                <Tooltip cursor={{ fill: '#f1f5f9' }} formatter={(v) => !isRpFormat ? formatNum(v) : formatRp(v)} contentStyle={{ backgroundColor: 'white', border: '1px solid #e2e8f0', borderRadius: '6px' }} />
                <Legend iconType="plainline" wrapperStyle={{ fontSize: '9px', paddingTop: '10px', color: '#1e293b' }} />
                
                {/* RENDER BAR YANG DISESUAIKAN */}
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 p-3 lg:p-4 2xl:p-6 font-mono text-xs">
      ...
      <div className="flex justify-end gap-2 mb-3 2xl:mb-4">
        <button onClick={() => router.push('/input')} className="flex items-center gap-1.5 px-2 py-1 2xl:px-3 2xl:py-1.5 bg-white border border-slate-200 rounded text-[9px] 2xl:text-[10px] font-black text-slate-600 hover:text-sky-700 hover:bg-sky-50 transition-all uppercase shadow-sm">
          <Edit3 size={10} /> Input Data
        </button>
        {userRole === 'super_admin' && (
          <button onClick={() => router.push('/admin')} className="flex items-center gap-1.5 px-2 py-1 2xl:px-3 2xl:py-1.5 bg-white border border-slate-200 rounded text-[9px] 2xl:text-[10px] font-black text-slate-600 hover:text-emerald-700 hover:bg-emerald-50 transition-all uppercase shadow-sm">
            <Settings size={10} /> Control Tower
          </button>
        )}
        <button onClick={handleLogout} className="flex items-center gap-1.5 px-2 py-1 2xl:px-3 2xl:py-1.5 bg-white border border-slate-200 rounded text-[9px] 2xl:text-[10px] font-black text-slate-600 hover:text-red-700 hover:bg-red-50 transition-all uppercase shadow-sm">
      ...
      <div className="mb-4 2xl:mb-6 flex flex-col lg:flex-row justify-between items-start lg:items-center border-b border-slate-200 pb-3 2xl:pb-4 gap-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 2xl:w-12 2xl:h-12 bg-white border border-slate-200 rounded shadow-sm flex items-center justify-center overflow-hidden shrink-0 p-1">
            {activePropLogo ? (
              <img src={activePropLogo} alt="Logo" className="max-w-full max-h-full object-contain" />
            ) : (
              <div className="bg-slate-900 text-white w-full h-full flex items-center justify-center rounded-sm"><LayoutGrid size={16} /></div>
            )}
          </div>
          <div>
        ...
        <div className="flex flex-wrap bg-white border border-slate-200 p-1 rounded items-center gap-1.5 shadow-sm">
          {userRole === 'super_admin' && (
            <>
              <select value={activePropId} onChange={handlePropChange} className="bg-sky-50 text-sky-800 font-black border-none text-[10px] 2xl:text-xs focus:outline-none cursor-pointer px-2 py-1 rounded uppercase">
          ...
          <div className="flex items-center bg-slate-100 rounded p-0.5 border border-slate-200">
            ...
            {compareMode === "YOY" && (
               <select value={compareYear} onChange={(e) => setCompareYear(e.target.value)} className="ml-1 bg-white text-indigo-700 font-black rounded-sm text-[9px] 2xl:text-[10px] p-0.5 border border-indigo-200">
      ...
      {/* KPI CARDS */}
      <div className={`grid grid-cols-2 md:grid-cols-4 gap-2 2xl:gap-4 mb-4 2xl:mb-6 transition-opacity duration-300 ${isLoadingDB ? 'opacity-30' : 'opacity-100'}`}>
        <div className="bg-white border-l-4 border-l-sky-600 p-2.5 2xl:p-4 rounded shadow-sm border border-slate-200">
          <p className="flex items-center gap-1 text-slate-500 font-black uppercase text-[8px] 2xl:text-[9px] mb-1"><Briefcase size={8} /> GOP</p>
          <p className="text-sm 2xl:text-lg font-black text-slate-900">{formatRp(mainStats.gop_act)}</p>
        </div>
        <div className="bg-white border-l-4 border-l-emerald-600 p-2.5 2xl:p-4 rounded shadow-sm border border-slate-200">
          <p className="flex items-center gap-1 text-slate-500 font-black uppercase text-[8px] 2xl:text-[9px] mb-1"><Users size={8} /> OCCUPANCY</p>
          <p className="text-sm 2xl:text-lg font-black text-slate-900">{mainStats.occAct.toFixed(1)}%</p>
        </div>
        <div className="bg-white border-l-4 border-l-indigo-600 p-2.5 2xl:p-4 rounded shadow-sm border border-slate-200">
          <p className="flex items-center gap-1 text-slate-500 font-black uppercase text-[8px] 2xl:text-[9px] mb-1"><Activity size={8} /> REVPAR</p>
          <p className="text-sm 2xl:text-lg font-black text-slate-900">{formatRp(mainStats.revparAct)}</p>
        </div>
        <div className="bg-white border-l-4 border-l-red-600 p-2.5 2xl:p-4 rounded shadow-sm border border-slate-200">
          <p className="flex items-center gap-1 text-slate-500 font-black uppercase text-[8px] 2xl:text-[9px] mb-1"><Percent size={8} /> F&B COST</p>
          <p className={`text-sm 2xl:text-lg font-black ${mainStats.fbCostRatio > 32 ? 'text-red-600' : 'text-emerald-600'}`}>{mainStats.fbCostRatio.toFixed(1)}%</p>
        </div>
      </div>
      ...
      <div className={`grid grid-cols-1 lg:grid-cols-12 gap-4 2xl:gap-8 transition-opacity duration-300 ${isLoadingDB ? 'opacity-30 pointer-events-none' : 'opacity-100'}`}>
        ...
        <div className="lg:col-span-2 space-y-2 border-r border-slate-200 pr-2 2xl:pr-4">
          ...
          <ul className="space-y-1">
            {indexMenuRev.map((item) => (
              <li key={item.id} onClick={() => setActiveDept(item.id)} 
                  className={`text-[9px] 2xl:text-[11px] font-black cursor-pointer px-2 py-1.5 2xl:px-3 2xl:py-2 rounded transition-all 
                  ${activeDept === item.id ? 'bg-sky-900 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100'}`}>
                {item.label}
              </li>
            ))}
          </ul>
        ...
        <div className="lg:col-span-10">
          <div className="flex justify-between items-center mb-3 2xl:mb-5 border-b border-slate-200 pb-2">
            ...
            {!isCostOrGop && (
               <button onClick={() => setShowBudget(!showBudget)} className="text-[8px] 2xl:text-[10px] font-black uppercase text-sky-700 bg-sky-50 px-2 py-1 2xl:px-3 2xl:py-1.5 rounded border border-sky-100">
